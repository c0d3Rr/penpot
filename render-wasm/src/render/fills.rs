use crate::shapes::{Fill, ImageFill, Type, Shape, Bool, Rect};
...
pub fn render(render_state: &mut RenderState, shape: &Shape, fill: &Fill) {
    ...
    match (fill, &shape.shape_type) {
        ...
        (_, Type::Rect(Rect { corners: None, .. })) => {
            canvas.draw_rect(selrect, &fill.to_paint(&selrect));
        }
        (_, Type::Rect(Rect { corners: Some(corners), ..})) => {
        ...
        (_, Type::Path(path)) | (_, Type::Bool(Bool { path, .. })) => {
